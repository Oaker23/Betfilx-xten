...
$sql= "SELECT * FROM `promotion` WHERE `p_name`='ฝากครั้งแรกของวัน' or `p_name`='รับทุกครั้งที่ฝากเงิน'";
$result = $server->query($sql);
$res=[];
...
$query = $server->query('SELECT * FROM history_promotion WHERE date_check = "'.$date_check.'" and phone="'.$_SESSION["phone"].'"');
$check1 = $query->num_rows;
  ...
  <div id="app">
    <main role="main">
      <div class="container content" v-cloak>
        ...
        <section class="bonus">
          <h3 class="text-center">โบนัส รายวัน</h3>

          
          <?php
          // if ($status!=3) {
          //   echo '<div class="alert alert-danger text-center mt-3" role="alert">คุณไม่สมารถรับโปรโมชั่นนี้ได้ ต้องฝากเงินเล่นและถอนก่อน</div>';
          // }

          if($check1 >=1){
            echo '<div class="alert alert-danger text-center mt-3" role="alert">คุณรับโบนัส ของวันนี้แล้ว!</div>';
            
          }
          
          if ($check==0) {
            echo '<center style="margin-top: 100px;">ไม่มีข้อมูล</center>';
          }
          ?>
