...
if($searchValue != ''){
	...
	c.username_game like '%".$useWord."%' or 
...
$sel = mysqli_query($server,"select count(*) as allcount from log_member WHERE 1 ");
$records = mysqli_fetch_assoc($sel);
$totalRecords = $records['allcount'];

## Total number of records with filtering
$sel = mysqli_query($server,"select count(*) as allcount from log_member a 
							 left join member c on (a.user_member = c.phone)
							 left join log_detail d on (a.detail_code = d.code) WHERE 1 ".$searchQuery);
$records = mysqli_fetch_assoc($sel);
$totalRecordwithFilter = $records['allcount'];

## Fetch records
$empQuery = "select a.id, c.username_game as user_member, c.fname as user_member_name, a.detail_code, d.name as detail_name, a.value1, a.value2, a.value3, a.value4, a.create_date
			 from log_member a 
			 left join member c on (a.user_member = c.phone)
			 left join log_detail d on (a.detail_code = d.code) 
