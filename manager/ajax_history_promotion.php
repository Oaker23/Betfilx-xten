...
if($searchValue != ''){
	...
	$searchQuery = " and (name like '%".$useWord."%' or 
    username_game like '%".$useWord."%' or 
    promotion like '%".$useWord."%' or 
    turnover like '%".$useWord."%' or 
    phone like'%".$useWord."%' ) ";
}

## Total number of records without filtering
$sel = mysqli_query($server,"select count(*) as allcount from history_promotion ");
$records = mysqli_fetch_assoc($sel);
$totalRecords = $records['allcount'];

## Total number of records with filtering
$sel = mysqli_query($server,"select count(*) as allcount from history_promotion WHERE 1 ".$searchQuery);
$records = mysqli_fetch_assoc($sel);
$totalRecordwithFilter = $records['allcount'];

## Fetch records
$empQuery = "select * from history_promotion WHERE 1 ".$searchQuery." order by ".$columnName." ".$columnSortOrder." limit ".$row.",".$rowperpage;
...
if ($row['username_game']!="") {
   $data[] = array(
            ...
            "name"=>$row['name'],
            "phone"=>$row['phone'],
            "promotion"=>number_format($row['promotion'],2),
            "credit"=>number_format($row['credit'],2),
            "turnover"=>number_format($row['turnover'],2),
        );
}
}
