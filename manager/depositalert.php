        ...
        <div class="col">
          <div class="card mb-4">
            <div class="card-header">
              <h4>รายการ ฝากเงินไม่สำเร็จ</h4> <h5>*กรณีรอยอดเงินไม่เข้าเกิน 5 นาทีจึงทำรายการหน้านี้</h5>
            </div>
            <div class="card-body">
...
function add_credit(data) {
  ...
  $.ajax({
    url:'add_credit_deposit_alert.php',
    type:'POST',
    data:{
      id:id
    }, 
...
function update_credit() {
  var id_credit=$("#id_credit").val();
   var phone=$("#phoneDepositAlert").val();
   $.ajax({
    url:'actionNew.php?update_credit_deposit_alert',
    type:'POST',
    data:{
      id_credit:id_credit,
      phone:phone
  ...
  $(document).ready(function(){
   $('#tableWithdraw').DataTable({
    ...
    {data: "id" , render : function ( data, type, row, meta ) {
      ...
      {data: "id" , render : function ( data, type, row, meta ) {
        return type === 'display'  ?
        "<button onclick='add_credit(`"+data+"`)' class='btn btn-primary'><i class='fas fa-plus'></i>&nbsp;เพิ่ม&nbsp;เครดิต</button>" :data;    }},
...
<div class="modal fade" id="add_credit" data-bs-backdrop="static" data-bs-keyboard="false" tabindex="-1" aria-labelledby="staticBackdropLabel" aria-hidden="true" style="margin-top: 200px;">
  <div class="modal-dialog">
    <div class="modal-content">
      ...
      <div class="modal-footer">
        <button type="button" class="btn btn-primary" onclick="update_credit()"><i class="fas fa-plus"></i> เพิ่ม</button>
        <button type="button" class="btn btn-danger" data-bs-dismiss="modal">ปืด</button>
