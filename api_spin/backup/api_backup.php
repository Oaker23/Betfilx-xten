...
$wheel['item'][5]['percent'] = $arrWheel[4]['percent'];
    ...
    function insertHistory($username_game,$date_check, $time_check, $phone, $bet, $credit, $status)
    {
        include("config.php");
        $sql="INSERT INTO `history_wheel`(`date_check`, `time_check`, `phone`, `bet`, `credit`, `status`, `username_game`) VALUES ('".$date_check."','".$time_check."','".$phone."', '".$bet."','".$credit."','".$status."','".$username_game."')";
        $server->query($sql);
    }
...
if(isset($_POST['nonce'])){
    ...
    if ($rand >= $wheel['unlucky']['percent']) {
        ...
        if ($status=='success') {
            ...
            insertHistory($username_game_only,$date_check, $time_check, $userPhone, $bet, $credit, "ได้รางวัล");
        }
        ...
        if ($status=='success') {
            ...
            $bet = $wheel['bet'];
            $username_game_only = $user['username_game'];
            insertHistory($username_game_only,$date_check, $time_check, $userPhone, $bet, 0, "ไม่ได้รางวัล");
        }
    }
    // หักเครดิต
