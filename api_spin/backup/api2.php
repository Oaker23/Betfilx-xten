...
$wheel['item'][5]['percent'] = $arrWheel[4]['percent'];
    ...
    function insertHistory($username_game_only, $date_check_now, $time_check_now, $phone, $bet, $credit, $status)
    {
        include("config.php");
        $sql="INSERT INTO `history_wheel`(`date_check`, `time_check`, `phone`, `bet`, `credit`, `status`, `username_game`) VALUES ('".$date_check_now."','".$time_check_now."','".$phone."', '".$bet."','".$credit."','".$status."','".$username_game_only."')";
        $server->query($sql);
    }
...
if(isset($_POST['nonce'])){
    ...
    if ($rand >= $wheel['unlucky']['percent']) {
        ...
        $server->query($sql);
        $bet = $wheel['bet'];
        $status = json_decode($api->deposit($username_game_only,1),true);
